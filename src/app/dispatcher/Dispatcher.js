...
class Dispatcher {
	 ...
	 * @returns {Dispatcher}
	 */
	register (actionId, callback, store) {
	...
	unregister (actionId, callback) {
		assert(
			...
			'Dispatcher.unregister(...): This callback was never registered' +
			' with id `' + actionId + '`'
		);
	...
	dispatch (payload) {
		...
		try {
			this._callbacks[this._currentActionId()].forEach(function (item,
				index) {
				if (!this._isPending[index]) {
					this._invokeCallback(index);
				}
	 ...
	 * @private
	 */
	_callbackExists (actionId, callback) {
